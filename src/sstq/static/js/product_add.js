...
const capturedImage = document.getElementById("add-captured-image");
const captureLink = document.getElementById("open-picture-capture");
...
async function validateBarcode(barcode) {
  const response = await fetch("/validate_barcode", {
    method: "POST",
...
function updateCaptureLink() {
  ...
  const url = new URL(captureLink.href, window.location.origin);
  url.searchParams.set("barcode", barcode);
  captureLink.href = `${url.pathname}${url.search}`;
}
...
async function uploadSelectedImage() {
  ...
  if (!file) {
    if (imageStatus) imageStatus.textContent = "Please choose an image file first.";
    return;
  }

  if (imageStatus) imageStatus.textContent = "Uploading image to cache...";
  try {
    const data = await uploadImageBlob(file);
    if (!data.success) {
      if (imageStatus) imageStatus.textContent = data.message || "Image upload failed.";
      return;
    }
    applyUploadedImageUrl(data.image_url);
    if (imageStatus) imageStatus.textContent = "Cached image ready. Save the product to publish it.";
  } catch {
    if (imageStatus) imageStatus.textContent = "Image upload failed.";
  }
}
  ...
  if (field.type === "file_reference") {
    ...
    fileInput.addEventListener("change", async () => {
      ...
      try {
        ...
        uploadStatus.textContent = "Cached PDF ready. Save the product to publish it.";
      } catch {
  ...
  const renderRows = () => {
    ...
    state.forEach((row, index) => {
      ...
      config.fields.forEach((field) => {
        body.appendChild(
          createField(field, row[field.name], (name, fieldValue) => {
            row[name] = fieldValue;
            syncTextarea();
            summary.textContent = config.title(row, index);
          }),
...
barcodeInput?.addEventListener("input", updateCaptureLink);
updateCaptureLink();
...
addForm?.addEventListener("submit", async (event) => {
  ...
  if (!barcodeValid.valid) {
    if (barcodeError) {
      barcodeError.textContent = barcodeValid.message || "Invalid barcode.";
